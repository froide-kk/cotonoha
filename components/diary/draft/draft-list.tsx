  ...
  useEffect(() => {
    async function fetchDrafts() {
      try {
        ...
        setError(err instanceof Error ? err.message : "下書きの取得中にエラーが発生しました")
        console.error(err)
      } finally {
        setIsLoading(false)
      }
      ...
      {isExpanded && (
        <Card>
          <CardContent className="p-4">
            {isLoading ? (
              <div className="py-4 text-center text-muted-foreground">下書きを読み込んでいます...</div>
            ) : error ? (
              <div className="py-4 text-center text-destructive">{error}</div>
            ) : drafts.length === 0 ? (
              <div className="py-4 text-center text-muted-foreground">下書きはありません</div>
            ) : (
                    ...
                    >
                      <div className="flex flex-col items-start">
                        ...
                        <span className="text-sm text-gray-500 truncate max-w-full">
                          {draft.content.length > 50 ? `${draft.content.substring(0, 50)}...` : draft.content}
                        </span>
